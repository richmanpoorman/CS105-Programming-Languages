...
val string_list_toString = Unit.listString Unit.stringString
        ...
        val () = 
            ...
            [] 
        val () =
            Unit.checkExpectWith string_list_toString 
            "List of multiple symbols"
            (fn () => flattenSyms (SXS [SYM "A", SYM "B", SYM "C"]))
            ["A", "B", "C"]
        val () =
            Unit.checkExpectWith string_list_toString 
            "List with duplicate symbols"
            (fn () => flattenSyms (SXS [SYM "A", SYM "A", SYM "B", 
                                        SYM "C", SYM "B"]))
            ["A", "A", "B", "C", "B"]
        val () =
            Unit.checkExpectWith string_list_toString "List of mixed symbols"
            (fn () => flattenSyms (SXS [SYM "A", NUM 1, BOOL true, SYM "B", 
                                        SYM "C"]))
            ["A", "B", "C"]
        val () =
            Unit.checkExpectWith string_list_toString "Nested List"
            (fn () => flattenSyms (SXS [SXS [SYM "B", NUM 1, SXS []], 
                                        SYM "A"]))
            ["B", "A"]
